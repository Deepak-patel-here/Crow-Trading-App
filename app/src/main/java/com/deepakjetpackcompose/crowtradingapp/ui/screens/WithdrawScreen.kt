...
fun WithdrawScreen(
    ...
    LaunchedEffect(isLoading.value) {
        if (isLoading.value == UpdateBalance.Success) {
            navController.navigate(NavigationHelper.SuccessfulWithdrawScreen){
                popUpTo(NavigationHelper.WithdrawScreen(balance=balance)){
                    inclusive=true
                }
            }
        }
    ...
    )
    {
        ...
        Text(
            "Please Enter the Amount you want to WithDraw.", fontSize = 16.sp,
        ...
        Button(
            onClick = {
                ...
                if (amount.value.substring(1).toDoubleOrNull()?:0.00 <= balance.toDoubleOrNull()?:0.00) {

                    authViewModel.updateAccount(
                        amount = amount.value.substring(1),
                        sign = "-"
                    ) { success, msg ->
            ...
            Text("WithDraw", fontSize = 16.sp, color = Color.Black, fontWeight = FontWeight.Bold)
        }
